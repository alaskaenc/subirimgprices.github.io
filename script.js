...
const GITHUB_TOKEN = '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'; // Asegúrate de que sea correcto
...
async function uploadImageToGitHub(imageName, base64Content) {
  ...
  if (!response.ok) {
    const errorData = await response.json(); // Captura el error de la respuesta
...
async function updateJsonFile(price, imageName) {
  ...
  if (!jsonResponse.ok) {
    const errorData = await jsonResponse.json(); // Captura el error de la respuesta
    throw new Error(`Error al obtener el JSON: ${errorData.message}`);
  }
  ...
  if (!updateResponse.ok) {
    const errorData = await updateResponse.json(); // Captura el error de la respuesta
